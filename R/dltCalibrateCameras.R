dltCalibrateCameras <- function(coor.2d, nx, grid.size, c.run = FALSE, reduce.grid.dim = 3, 
	...
	if(length(grid.size) == 1){
		coor_3d_coeff <- transformPlanarCalibrationCoordinatesR(tpar=matrix(c(p_init),ncol=6, byrow = T), nx=nx, ny=ny, sx=grid.size[1])
	}else{
		coor_3d_coeff <- transformPlanarCalibrationCoordinatesR(tpar=matrix(c(p_init),ncol=6, byrow = T), nx=nx, ny=ny, sx=grid.size[1], sy=grid.size[2])
	}
